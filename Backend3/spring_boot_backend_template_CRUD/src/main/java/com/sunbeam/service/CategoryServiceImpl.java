...
//	}
	...
	@Override
	public CategoryResDto addNewCategoryWithImage(CategoryResDto dto, MultipartFile image) throws IOException {
		
		Category categoryEntity = mapper.map(dto,Category.class);
		imgHandlingService.uploadImage(categoryEntity, image);
		Category savedCategory = categoryDao.save(categoryEntity);
		return mapper.map(savedCategory, CategoryResDto.class);
	}

	public void uplodadAndSaveCategory(Category category,MultipartFile image)
	{
